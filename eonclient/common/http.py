...
from eonclient.openstack.common.py3kcompat import urlutils


LOG = logging.getLogger(__name__)
USER_AGENT = 'python-eonclient'
...
class HTTPClient(object):
    ...
    def _http_request(self, url, method, **kwargs):
        ...
        try:
            ...
            endpoint = self.endpoint
            message = ("BadStatusLine exception: %(e)s is occurred while "
                       "communicating with %(endpoint)s. Please check the "
                       "status of eon services" % dict(endpoint=endpoint, e=e))
            raise exc.CommunicationError(message=message)
